...
```{r load-bench, message = FALSE, warning = FALSE}
library(bench)
```
...
You may have some other ideas of how to solve this problem! How do we figure out which one is fastest, though? The `mark()` function from bench takes in different proposals to solve the same problem and returns a tibble with information about how long they took (among other things.)
...
```{r filter, eval = FALSE}
filter(mtcars_tbl, hp > 100)
```

...can be replaced by:

```{r filter-alt, eval = FALSE}
vec_slice(mtcars_tbl, mtcars$hp > 100)
```
...
res <-
  mark(
    ...
    `[.tbl_df` = mtcars_tbl[mtcars$hp > 100, ]
  ) %>%
...
Note that `[.tbl_df` sets `drop = FALSE`, so no need to manually specify that argument if you're used to doing so for `data.frame`s. Be sure that you're subsetting a tibble rather than a data frame, though.
