...
There's only really one function in waldo that you'll ever use: `waldo::compare()`. It's job is to take a pair of objects and succinctly display all differences. When comparing atomic vectors, `compare()` uses the [diffobj](https://github.com/brodieG/diffobj) package by Brodie Gaslam) to show additions, deletions, and changes:
