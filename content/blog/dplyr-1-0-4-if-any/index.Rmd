...
This release dramatically improves the performance of `across()` and introduces 
the new functions `if_all()` and `if_any()` as companions to `filter()`. 
...
The new `across()` function introduced as part of dplyr 1.0.0 is starting to be 
...
Although `if_all()` and `if_any()` were designed with `filter()` in mind, we then discovered
through [this github issue](https://github.com/tidyverse/dplyr/issues/5709) that they can be equally 
useful when used within `mutate()` and/or `summarise()`. 
