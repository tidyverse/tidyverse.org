...
`labelled_spss()` gains full vctrs support thanks to the hard work of [Danny Smith](https://github.com/gorcha). This means that `labelled_spss()` objects should now work seamlessly in dplyr 1.0.0, tidyr 1.0.0.
...
* Bundleed ReadStat has been updated to version 1.1.5 from 1.1.3 so 
  ...
  Probably biggest improvement is support for SAS binary (aka Ross) compression.
...
* `write_*()` now validates file and variable metadata with ReadStat and
  ...
  (e.g. the column name), make it easier to track down issues.
