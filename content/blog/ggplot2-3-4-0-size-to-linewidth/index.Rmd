...
the news paradigm. For example, take a look at this piece of old code:
...
but ultimately we of course wants users to migrate to the following
code:
...
> The last two plots are not equal because the `default` `linewidth`
> scale correctly use a linear transform instead of a square root
> transform
...
geom drawing circles and then update it to using linewidth instead:
...
a `linewidth` aesthetic. Second, we updates the `default_aes` to use
