...
<!-- -->
```
-   `dbAppendTable(),` `dbCreateTable()`, and `dbExecute` are now supported, and `dbGetQuery()`/`dbSendQuery()` support parameterised queries via the `params` argument.
    `dbReadTable()`, `dbWriteTable()`, `dbExistsTable()`, `dbRemoveTable()`, and `dbListFields()` now all work with `DBI::Id()`.
