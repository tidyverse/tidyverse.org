...
Both packages are implemented in Rust (built on the excellent [`saphyr`](https://github.com/saphyr-rs/saphyr) crate). They share the same design goals: predictable YAML 1.2 typing, explicit control over tag interpretation via handlers, and clean round-tripping of unhandled tags.
...
YAML 1.2 tightened up a number of ambiguous implicit conversions. In particular, plain scalars like `on`/`off`/`yes`/`no`/`y`/`n` are strings in the 1.2 core schema, and YAML 1.2 removed sexagesimal parsing (so values like `1:2` are not treated as numbers).
...
That makes YAML a better default for configuration files, front matter, and data interchange: fewer surprises and fewer "why did this become a boolean?" moments (or "why did this become a date?").
...
<span><span class='s'>---</span></span>
<span><span class='s'>title: Extracting YAML frontmatter</span></span>
<span><span class='s'>---</span></span>
...
<span><span class='c'>#&gt;  $ title: chr "Extracting YAML frontmatter"</span></span>
