...
Mostly, this change shouldn't be very noticeable when you're just using ggplot2 for building regular plots. At best, you may notice that we're more strictly enforcing types for certain arguments. For example, much ludicrous input is now rejected right away. This is due to how properties in S7 work, which get validated when a new object is instantiated.
...
We have put various pieces of backwards compatibility in to not break many packages that assumed the S3 structures of ggplot2. For example, we still return the data property with `ggplot()$data`, whereas the S7 way of accessing this should be `ggplot()@data`. Expect these to be phased out over time in favour of S7.

## Theme improvements

Themes in ggplot2 have long served the role of capturing any non-data aspects of styling plots. We had come to realise that the default look of layers, from what the default shape of points is to what the default colour palette is, are also not truly data-driven choices. The idea to put these defaults into themes has been around for a while and Dana Page Seidel did pioneering work implementing this as early as 2018. Now, years of waiting have come to fruition and we're proud to announce this new functionality.

### Ink and paper

The way layer defaults are implemented differ slightly from the aesthetics you know and love (or on rare occasions: hate). In themes, the graphical properties are oriented around their role rather 1:1 theme-to-layer translation. Consider the `fill` in a boxplot. The amount of boxplot `fill` that is displayed does not correspond to the data. It typically even blends in with the background. In contrast, the `fill` in an area or bar chart directly corresponds with data: the more ink is present, the higher are data values (typically). For this reason, we distinguish foreground colours (`ink`) and background colours (`paper`). To accommodate special cases, we also added a third `accent` option.
...
In addition to the defaults for layers, also default palettes are now encapsulated in the theme. The relevant theme settings have the pattern `palette.{aesthetic}.{type}`, where `type` can be either discrete or continuous. This allows you to coordinate your colour palettes with the rest of the theme.
...
-   [`margin_auto()`](https://ggplot2.tidyverse.org/reference/element.html) sets the margins in a CSS-like fashion similar to the [`padding`](https://developer.mozilla.org/en-US/docs/Web/CSS/padding) property.
...
Also [`geom_label()`](https://ggplot2.tidyverse.org/reference/geom_text.html) has new styling options. It now has a `linetype` and `linewidth` aesthetic, which can be mapped from the data. The `linewidth` aesthetic replaces the `label.size` argument, which used determine the line width of the label border. In addition to the new aesthetics, [`geom_label()`](https://ggplot2.tidyverse.org/reference/geom_text.html) has two new arguments: `border.colour` and `text.colour` which set the colour for the border and text respectively. When these are set, it overrules the `colour` aesthetic for a part of the label. In the plot below, we fix the `text.colour` to black, so the `colour` aesthetic applies to the border, not the text.
...
<span><span class='c'>#&gt;             ...))</span></span>
<span><span class='c'>#&gt; &#125;</span></span>
<span><span class='c'>#&gt; &lt;environment: 0x00000228711a9380&gt;</span></span>
<span></span></code></pre>
...
<span>  <span class='nf'><a href='https://ggplot2.tidyverse.org/reference/draw_element.html'>element_grob</a></span><span class='o'>(</span></span>
...
[^1]: Aesthetics of the position adjustment are not be confused with position aesthetics. Position aesthetics like `x` and `y` are transformed by a scale, whereas aesthetics of the position adjustment like `nudge_x` and `nudge_y` are not (akin to `width` and `height`).
