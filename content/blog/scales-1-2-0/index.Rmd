...
This is useful for scales that span multiple orders of magnitude. Take the following two examples which don't get great labels by default:
...
Finally, `label_dollar()` recieves a couple of small improvements. The  the `prefix` is now placed before negative sign, rather than before it, yielding (e.g) the correct `-$1` instead of `$-1`:
...
It also longer uses its own `negative_parens` argument but instead inherits the new `style_negative` argument from `label_number()`:
