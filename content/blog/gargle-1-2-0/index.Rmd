...
Users of those packages, especially those who take an interest in the details around auth, may want to read on.
...
# TODO: probably delete this chunk; no one uses gargle directly
library(gargle)
```
...
This reduces the chance of accidentally sending tokens to remote servers (like DropBox or GitHub), increases the ability to reuse tokens across multiple R projects, and makes it easier to use multiple identities within a single project.
...
* gargle is quite conservative about using cached tokens and likes to have
  ...
  - In a non-interactive context, gargle will use a cached token, if it can
    ...
    unambiguous in the code. This comes up, for example, when rendering R
    Markdown documents.
...
User tokens created with gargle are the result of so-called "three-legged OAuth", where the three legs are the user, Google, and an OAuth client (id and secret).
...
We use nicknames to keep track of the built-in client; the new one is known as "tidyverse-calliope", which is replacing the older "tidyverse-clio".
...
Not all wrapper packages use this functionality, but, in particular, googlesheets4 *does*.
...
When you hit the Sheets "per user" or "per project" limit, requests fail with the error code `429 RESOURCE_EXHAUSTED`.[^3]

[^3]: There is a limit of "500 requests per 100 seconds per project".
...
`request_retry()` is a drop-in substitute for `request_make()` that uses (modified) exponential backoff to retry requests that fail with error `429`.
`request_retry()` even has some special knowledge about the Sheets limits and, in some cases, will wait 100 seconds for the user's quota to reset.
