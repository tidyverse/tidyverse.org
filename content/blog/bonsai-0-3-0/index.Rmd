...
That best RMSE value of `r round(show_best(res_lr, metric = "rmse")$mean[1], 3)` gives us a baseline to work with, and the best R-squared `r round(show_best(res_lr, metric = "rsq")$mean[1], 3)` seems like a good start.
...
* **Gradient boosted tree with LightGBM**. Best RMSE: `r round(show_best(res_lgb, metric = "rmse")$mean[1], 3)`. Best R-squared: `r round(show_best(res_lgb, metric = "rsq")$mean[1], 3)`.
* **Partial least squares regression**. Best RMSE: `r round(show_best(res_pls, metric = "rmse")$mean[1], 3)`. Best R-squared: `r round(show_best(res_pls, metric = "rsq")$mean[1], 3)`.
* **Support vector machine**. Best RMSE: `r round(show_best(res_svm, metric = "rmse")$mean[1], 3)`. Best R-squared: `r round(show_best(res_svm, metric = "rsq")$mean[1], 3)`.
...
Holy smokes. The best RMSE from aorsf is `r round(show_best(res_aorsf, metric = "rmse")$mean[1], 3)`, much more performant than the previous best RMSE from the elastic net with `r round(show_best(res_lr, metric = "rmse")$mean[1], 3)`, and the best R-squared is `r round(show_best(res_aorsf, metric = "rsq")$mean[1], 3)`, much stronger than the previous best (also from the elastic net) of `r round(show_best(res_lr, metric = "rsq")$mean[1], 3)`.
