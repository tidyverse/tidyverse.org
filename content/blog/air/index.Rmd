...
I find that it's much easier to show what Air can do rather than tell, so we'll start with a few examples.
Let's jump into Positron with some unformatted dplyr code for `case_when()`.
Saving the file (yep, that's it!) calls Air, which automatically and instantaneously styles the code.

<video controls autoplay loop muted width="100%" src="video/case-when.mov" style="border: 2px solid #CCC;">

</video>

Next, let's go over to RStudio.
Here we've got a pipe chain that could use a little styling.
...
Created by [Lorenz Walthert](https://github.com/lorenzwalthert) and [Kirill Müller](https://github.com/krlmlr), styler proved that the R community does care about how their code is formatted, and had been the primary implementation of the [tidyverse style guide](https://style.tidyverse.org/) for many years.
...
-   Air does not require R to run.
    ...
    This makes Air easily usable across IDEs or on CI with very little setup required.
...
One big goal for Air is for the "format on save" gesture to be imperceptibly fast, encouraging you to keep it on at all times.
...
Out of curiosity, we ran Air over all \~900 R files in base R and it finished in under 2 seconds.
We didn't try this with styler :)
