...
a package powered by [DuckDB](https://duckdb.org/).
...
We could transform the data using dplyr but we could also transform it using a tool that'll scale well to ever larger data, using duckplyr.
...
A drawback of analyzing data larger than memory with duckplyr is that the limitations of duckplyr won't be compensated by fallbacks, since fallbacks to dplyr necessitate putting data into memory.
