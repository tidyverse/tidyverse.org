...
Then we'll get to the good stuff: improvements to the map family, new `keep_at()` and `discard_at()`, and improvements to flattening and simplification.
...
Making these fixes helped give me confidence that while we're deprecating quite a few functions and changing a few special cases, it shouldn't affect too much code in the wild.
...
Now `default` is only use for `NULL`s and absent elements:
...
We've tweaked the map family of functions to be more consistent with general tidyverse coercion and recycling, as implemented by the [vctrs](https://vctrs.r-lib.org) package.
