...
I am very excited to introduce work currently underway on the probably package.  We are looking to create 
early awareness and receive feedback from the community. That is why the 
enhancements discussed here are not yet on CRAN. 
...
*The goal of model calibration is to ensure that the estimated class probabilities are
...
the probably package. The enhancements are centered around plotting functions meant for
...
we will use `segment_logistic`, an example data set that contains predicted probabilities and classes from a logistic regression model for a binary outcome `Class`, taking values `"good"` or `"bad"`.
predictions, and their probabilities. `Class` contains the outcome of 
`.pred_good` contains the probability that the event is "good". 
...
The most common approach is to group the probabilities
