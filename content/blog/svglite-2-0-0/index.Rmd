...
bench::mark("svglite()" = svglite_test(), 
            "svg()" = svg_test(), 
            min_iterations = 250) %>% 
  plot(type = "ridge") + 
  labs(x = NULL, y = NULL)
...
For more or less the same reasons as given above for rendering speed, the final file size of the rendered SVG can be very important when streaming graphics from a web server.
Smaller files result in lower latency.
svglite has always produced smaller files than the `svg()` device, but this has been improved on further with this release, mainly by being smarter about how to handle clipping regions in the plot.
...
The main reason for the huge difference in size between the output of `svg()` and `svglite()` is that the former encodes text as polygons, whereas the later simply encodes it as text and relies on the svg renderer (usually a browser or a vector drawing program) to figure out the look of the text.
There are several additional upsides to the latter approach, and one major downside.
Let us get the bad out of the way first: Encoding text as text means that the final look depend on the implementation of the svg renderer as well as the fonts available on the system where it is rendered.
...
Now for the good stuff, and the subject of this section.
...
2.  Fixing of the text width is now optional.
    ...
    svglite has solved this by calculating a good guess at the width and using the `textLength` attribute of the `<text>` tag to force the renderer to make the text fit into that space.
