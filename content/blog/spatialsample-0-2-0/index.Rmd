...
And that brings us to the first new feature in this release of spatialsample: `spatial_clustering_cv()` now supports sf objects, and will calculate distances in a way that respects coordinate reference systems (including using the s2 geometry library for geographic coordinate reference systems):
...
In addition to supporting more types of data, `spatial_clustering_cv()` has also been extended to support more types of clustering. Use the `cluster_function` argument to use hierarchical clustering via `hclust()` instead of the default `kmeans()`-based clusters:
