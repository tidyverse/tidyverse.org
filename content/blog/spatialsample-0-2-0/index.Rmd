...
In addition to supporting more types of data, `spatial_clustering_cv()` has also been extended to support more types of clustering. Set the `cluster_function` argument to `"hclust"` for hierarchical clustering via `hclust()` instead of the default `kmeans()`-based clusters:
