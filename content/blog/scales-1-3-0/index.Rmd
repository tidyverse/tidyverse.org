...
While scales have had rudimentary support for objects from the hms package it has not included basic support for difftime objects from base. This is now rectified with the introduction of `label_timespan()`, `breaks_timespan()`, and `transform_timespan()`. While the labels and breaks function can be used on their own, all the behavior is encapsulated in the timespan transform object which is kin to `transform_hms()`.

```{r}
library(ggplot2)

events <- data.frame(
  time = runif(30, max = 200),
  magnitude = rnorm(30) + 2
)

ggplot(events) + 
  geom_point(aes(time, y = 0, size = magnitude), position = position_jitter(height = 0)) + 
  scale_x_continuous(trans = transform_timespan(), limits = c(0, NA))
```
