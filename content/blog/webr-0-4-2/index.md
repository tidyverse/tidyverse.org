...
We're totally stoked to announce the release of [webr](https://docs.r-wasm.org/webr/latest/) 0.4.2!

It's been a little while since I've written about webR here, and there's been a few releases between my last post and this one. In this post I'll cover some of the exciting changes to the core webR distribution, and also include some interesting tidbits for JavaScript developers using webR in their own applications. You can see a full list of changes in the [release notes](https://github.com/r-wasm/webr/releases).

Subsequent posts in this series will cover updates to Shinylive for R, and introduce new Quarto extensions using the power of webR and WebAssembly to elevate your documents with interactivity.

## Supporting HTML and widget display

The base R distribution is designed to be run in a text console, but some options are provided for system-dependent display of content. For example, in a previous version of webR we implemented the [`pager`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/file.show.html) option so that R's help system can be better displayed within the [webR application](https://webr.r-wasm.org/latest/). With this, we can show R function and package documentation outside of the text console in dedicated tabbed windows.
...
This gives us the ability to show a tabular data viewer for `data.frame`-like R objects and an `iframe` based HTML content viewer, opening the way to displaying dynamic web-based output from R packages like [`leaflet`](https://rstudio.github.io/leaflet/articles/leaflet.html) and [`gt`](https://gt.rstudio.com/).
...
The implementation of the `viewer` mechanism is fairly general, making use of webR's [output messages](https://docs.r-wasm.org/webr/latest/communication.html#output-messages) to send the required information to the main JavaScript thread for display. That way, any application using webR may choose how to listen for those messages and show the resulting content on the page. We'll see this again in a later post where I talk about using webR to display dynamic content in Quarto extensions.
...
The webR app's code editor is powered by [CodeMirror](https://codemirror.net/) with R parsing provided by the [codemirror-lang-r](https://github.com/TravisYeah/lang-r) package. CodeMirror's extensibility is excellent, and the library is well suited for integrating into a wider project. However, we noticed that the `codemirror-lang-r` package had a few issues highlighting certain types of R syntax. In particular, in our application highlighting matrix operations such as `%*%` would crash the parser!
...
A nice side-effect of the work in the previous section is that with the changes in place [mounting filesystem data with `WORKERFS`](https://github.com/r-wasm/webr/issues/328) now also works correctly under Node.js, fixing a fairly painful and long-standing bug for our server-side users of webR.
...
You can try it out right here! Any files written to the `/data` directory in the interactive R console below should be persisted. The first time you load this page, the directory will be empty. However, if files are written they will remain available after you refresh the page or revisit in the future.

<div class="highlight">

{{< webr-editor code=`install.packages('cli', quiet = TRUE)\n\nfiles <- list.files("/data")\nfiles\n\nif (length(files) == 0) {\n  cli::cli_alert_warning("No files found in '/data', I'll create one...")\n  write.csv(mtcars, "/data/mtcars.csv")\n} else {\n  cli::cli_alert_success("Nice! Some existing files were found in '/data'.")\n}` width=504 height=311.472 >}}

</div>

It should be noted that filesystem data stored in an IndexedDB database can only be accessed within the current origin, loosely defined as the current web page (sub)domain and port. Also, browsers may decide the amount of storage space provided, what content is deleted when quotas are reached, and when exactly that deletion occurs. In private browsing mode, for example, data is usually removed when the private session ends.
...
For these reasons, the `PostMessage` communication channel is now the default fallback when the web page is not cross-origin isolated. The `ServiceWorker` channel has been deprecated, it will continue to be available in the short-term but will eventually be removed in a future version of webR.

### API additions

We've made some minor additions to the webR TypeScript/JavaScript API. There's nothing ground breaking here, but some new tools that we hope to be useful.

#### Report current version

With the aim of providing functionality similar to the [`packageVersion()`](https://rdrr.io/r/utils/packageDescription.html) R function, the version of the currently running webR session may now be obtained from the JavaScript environment.
...
In a previous version of webR, we introduced creating new R `data.frame` objects from JavaScript using the generic `RObject` constructor.
...
With the generic constructor JavaScript objects are reserved for creating an R `data.frame`. For example, if you would prefer to make an R list object the generic constructor throws an exception. You must instead be explicit by using the `RList` constructor,
