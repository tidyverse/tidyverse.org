...
The new and shiny feature of the release is that patchwork now has native support for gt objects, making it possible to compose beautifully formatted tables together with your figures. This has been made possible through Teun Van den Brand effort to provide grob output to gt. While the support means that you can now pass in gt objects to `wrap_elements()` in the same way as other supported data types, it goes one step further, using the semantics of the table design to add table specific formatting options through the new `wrap_table()` function.
