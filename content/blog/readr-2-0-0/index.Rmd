...
Or the new style by calling `summary()` on the specficiation object.
```{r}
summary(spec(x))
```
...
When files with these encodings are read they are automatically converted to UTF-8 internally in an efficeint streaming fashion.
