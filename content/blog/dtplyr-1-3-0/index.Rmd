...
To avoid this problem, we have removed those S3 methods and now must explicitly opt-in to dtplyr translations by using `lazy_dt()`.
...
-   Where possible, `select()` now drops columns by reference (i.e. with `var := NULL`).

-   `slice()` uses an intermediate variable to reduce computation time of row selection.

## Acknowledgements

A massive thanks to [Mark Fairbanks](https://github.com/markfairbanks) who did most of the work for this releases, ably aided by the other dplyr maintainers [\@eutwt](https://github.com/eutwt) and [Maximilian Girlich](https://github.com/mgirlich).
