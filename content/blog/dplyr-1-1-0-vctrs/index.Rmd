...
This post is all about updates to vector functions, like `case_when()` and `between()`.
All of the dplyr vector-style functions are now backed by vctrs, which typically results in better error messages, better performance, and greater versatility.
...
Like me, you've probably forgotten that `case_when()` is strict about the types on the right-hand side of the `~`, which means that I needed to use `NA_character_` here instead of `NA`.
We don't think most R users need to care about typed missing values, so luckily the switch to vctrs means that the above code "just works" now:
...
In this case, we have a fallthrough "default" captured by `TRUE ~`.
...
`.default` will always be considered the last condition, regardless of where you try to put it in the call to `case_when()`.
...
Remapping values in this way is actually so common that SQL gives it its own name - the "simple" case statement.
To make this even simpler (and faster), we've taken out some of the repetition involved with `x %in%` by introducing `case_match()`, which lets you specify one or more *values* on the left-hand side of the `~` rather than logical vectors.
...
I think that `case_match()` is particularly neat because it can be wrapped into an ad-hoc replacement helper if you just need to collapse or replace a few problematic values in a vector, but leave everything else unchanged:
...
At Posit, we have quarterly company update meetings.
...
We need a way to identify consecutive *runs* of when a single person is speaking, which is exactly what `consecutive_id()` is for!
...
<!-- -->
```
-   The ranking functions, like `dense_rank()`, now allow data frames as input as way to rank by multiple columns at once.

-   `first()`, `last()`, and `nth()` have all gained `na_rm` arguments since they are summary functions.
...
-   `na_if()` now casts `y` to the type of `x` to make it clear that it is type stable on `x`.
    ...
    You can also now replace `NaN` values.
