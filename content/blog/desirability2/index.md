...
We colored the point using the number of features used in the model. Fewer predictors are better and we'd like to factor that into the tuning parameter selection. 

To optimize them all at once, desirability functions are created that map their values to be between zero and one (with the latter being the most desirable). For the ROC scores, a value of 1.0 is best and maybe we wouldn't consider a model with an AUC less than 0.80. We can use the [`d_max()`](http://desirability2.tidymodels.org/reference/inline_desirability.html) function to translate these values to desirability: 
