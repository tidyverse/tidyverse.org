...
Notice that the return value of `sprintf()` and `snprintf()` are different. `sprintf()` 
returns the total number of characters written (excluding the null-terminator), while `snprintf()`
returns the number of character that would have been written had `n` been sufficiently large.
