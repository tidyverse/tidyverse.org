...
This blog post will give you an overview of the biggest changes (you can get a detailed list of all changes from the [release notes](%7B%20github_release%20%7D)).
...
Then, we'll give a quick overview of improvement to documentation and stringr's new license.
Lastly, we'll finish off by diving into the many small, but useful functions that we've accumulated in the three and half years since the last release.
...
We've tried to keep these small and we don't believe they'll affect much code in the wild (only affected \~20 of the \~1,600 packages that use stringr).
...
The biggest improvement is to `str_view()` which has gained a bunch of new features, including using the [cli](https://cli.r-lib.org/) package to work in more places.
