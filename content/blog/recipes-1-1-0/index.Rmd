...
Every recipe you create starts with a call to `recipe()`. We have relaxed the requirements of data frames, while making feedback more helpful when something goes wrong.
...
Another way to specify a recipe is to use `add_role()` and `update_role()`. But if you were not careful, you could end up in situations where the same variable is labeled as both the outcome and predictor.
