...
The package management functions provided by webR have been expanded and improved. Once enabled [^shims], `install.packages()`, `library()` and `require()` are shimmed so that installing or loading R packages automatically downloads WebAssembly binaries from the [webR package repository](https://repo.r-wasm.org). Also, it is no longer required to run the `library()` command a second time to subsequently load the package.

[^shims]: The package library shims are enabled by default in the webR app.
