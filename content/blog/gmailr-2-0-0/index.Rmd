...
That's because the scopes (think: "permissions") needed for the Gmail API are regarded as extremely sensitive, as well they should be.
...
* `gm_token_write()` + `gm_token_read()` is a new matched pair of functions that facilitate writing a token to disk, in an obfuscated or fully encrypted form, then reloading that token in a deployed data product or in CI.
