...
We're stoked to announce the release of [tabpfn](https://tabpfn.tidymodels.org/) 0.1.0. [TabPFN](https://github.com/PriorLabs/TabPFN) is a precompiled deep learning Python model for prediction. The _R package tabpfn_ is an interface to this model via [reticulate](https://rstudio.github.io/reticulate/). 
...
The syntax is idiomatic R: it supports fitting interfaces via data frames/vectors, formulas, and recipes. The standard R `predict()` method is used for prediction. `augment()` is also available for prediction. 
