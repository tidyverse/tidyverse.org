---
title: Announcing tibble 2.0.0
slug: tibble-2.0.0-announce
description: >
    Tibbles are a modern reimagining of the data frame, keeping what time has shown to be effective, and throwing out what is not, with nicer default output too! The upcoming 2.0.0 release adds a slew of features but also changes some internals, which in some cases might require adaptation. This blog post describes some of the stumbling blocks we've encountered when trying to adapt other packages to the dev version of tibble.
date: 2018-11-21
...
Version 2.0.0 of the _tibble_ package is almost ready for release.
Tibbles are a modern reimagining of the data frame, keeping what time has shown to be effective, and throwing out what is not, with nicer default output too! Grab the development version with:

```r
remotes::install_github("tidyverse/tibble")
```

We announce this release before actually submitting to CRAN, because some changes require attention from authors of packages that import or otherwise depend on _tibble_.
We would like to use this opportunity to gather feedback and perhaps rollback or soften some of the most disruptive changes.
This blog post is mostly interesting to package developers, new features and a higher-level overview will be presented in a separate blog post.

We tested ~3000 CRAN and Bioconductor packages that depend directly or indirectly on _tibble_ with `R CMD check`, and compared check results for the CRAN version with our results.
We notified the maintainers of the packages, and aim for a CRAN release before Christmas, just in time for [rstudio::conf](https://www.rstudio.com/conference/).

In the following we showcase the changes that affected many of the now failing packages that we analyzed, and suggest solutions.
For the full list of changes, features and bug fixes, please see the [release notes](https://github.com/tidyverse/tibble/tree/master/NEWS.md) for a complete list.
...
For instance, after importing data from a foreign format, only inspection of the data might reveal if the column is useful in the first place.
...
It is now harder (though not impossible) to construct a tibble with empty or duplicate column names.
...
If you must shut off the warning you still can.
