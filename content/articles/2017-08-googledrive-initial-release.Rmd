...
We'll quickly show off a few basic file operations and then clean up. This also demonstrates the file conversion features available when importing into and exporting out of native Google file types.

  * Make a folder.
  * Make a new Drive file (Google Sheet) by uploading a local file (csv). A few example files ship with googledrive and `drive_example()` is just a convenience function to get the local path on any system.
  * List the contents of the folder and reveal file path to confirm the new file is in the target folder.
  * Rename the file.
