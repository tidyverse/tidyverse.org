...
usethis provides a useful complement to WRE. Each function adds one specific piece of infrastructure to a package or project. Importantly, it documents this not (only) in prose, but also in executable R code. The implementation as discrete R functions also allows us to make specific bits of documentation readily available, through the normal R help mechanism. Even if you don't use the usethis functions all the time, they provide a handy reference.
