...
* `scope`. What is to be styled, i.g. just spacing or also indention, line breaks, tokens?
...
Hence, `scope < "tokens"` will never change code in the sense that the expression
remains the same before and after styling, as only spaces and line breaks between the tokens are modified. styler can internally validate the styling, so you can be sure that it has not broken your code. Although extensively tested, we recommend to inspect the styling result with `scope = "tokens"`, since in this case, an automatic validation is not possible.
...
Also, with `scope >= "line_breaks"`, `strict = TRUE` will break the lines 
in the following call, whereas `strict = FALSE` will leave them as is.
```{r}
style_text(
...
In addition, the `strict` option does also adds braces (if `scope = "tokens"`) to conditional statements if they are multi-line.
```{r}
style_text(
  "if (set_q(x))
    3 else 
...
styler can also format complicated expressions that involve line breaking and 
...
That was a quick overview of styler. Please feel free to grab styler from CRAN, run it on your own package, tell us what you think.
