...
* `nest()` and `unnest()` have been changed to match an emerging principle for 
  ...
  `chop()`/`unchop()`) reveal two simpler steps that underlying nesting.
...
You can see a list of all the other minor bug fixes and improvements in the [release notes](https://tidyr.tidyverse.org/dev/news/index.html#tidyr-1-0-0).
