...
We are very excited to announce that [testthat](http://testthat.r-lib.org) 2.0.0 is now available on CRAN! Testthat makes it easy to turn your existing informal tests into formal automated tests that you can rerun quickly and easily. testthat is used by over 2,600 CRAN packages and by many more [Github packages](https://github.com/search?q=filename%3Atestthat.R). Learn more about unit testing at <http://r-pkgs.had.co.nz/tests.html>. 
...
(Note that this is not tidy evaluation _per se_, but is closely related. This works a little differently in testthat to tidyverse packages because quoting is only used to generate the failure messages: see `?quasi_label()` for more details. At this time you can not unquote quosures.)
