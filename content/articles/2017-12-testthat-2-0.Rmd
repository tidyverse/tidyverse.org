...
* New setup and teardown tools provide rich ways of running code before and
...
* Quasiquotation support makes it easier to embed tests in for loops and
  functions and still get useful failure messages.

* Along with these new features we unforunately also had to make a few breaking 
...
We have identified a new family of expectations that compares the results of an expression to a known good value stored in a file. They are designed to be use in conjunction with git so that you can see what precisely has changed, and revert it if needed:
...
We've also added `expect_setequal()` to check if that compares two sets (stored in vectors), ignoring duplicates and differences in order. Finally we added few new helpers for skipping tests:
...
All expectations can now use [unquoting](https://adv-r.hadley.nz/tidy-eval) with `!!`. This makes it much easier to generate informative failure messages when running tests in a for loop or function. For example take this test:
...
Unfortunately it was necessary to make a few API breaking changes in testthat in order to make testthat more flexible for the future, and deal with some changes made to R. These changes affected around 1 in 40 packages  using testthat on CRAN, and all maintainers were warned a month in advanced. 
...
```
A big thanks goes to [Kirill Müller](https://github.com/krlmlr) for his help running R CMD check on all the packages that use testthat - in total he ran R CMD check over 10,000 times!
