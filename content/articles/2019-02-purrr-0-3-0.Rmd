...
* `reduce()` now support early termination of a computation. Just return a value wrapped in a `done()` to signal to `reduce()` that you're done.
...
An important improvement is that `modify()` is now a wrapper around `[[<-` rather than `[<-`. This should make it directly compatible with a larger set of vector classes.
