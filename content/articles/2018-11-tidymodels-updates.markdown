...
Combined with `metric_set()`, a new function for combining multiple metrics into one function call, this workflow makes calculating a large number of metrics over multiple resamples a quick task. I encourage you to check out the example section of [`metric_set()`'s help page](https://tidymodels.github.io/yardstick/reference/metric_set.html) if you are interested in learning more.
