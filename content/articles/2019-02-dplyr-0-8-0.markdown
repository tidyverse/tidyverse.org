...
```
  ...
  - [group_trim()](https://dplyr.tidyverse.org/reference/group_trim.html) drops unused levels of factors
  that are used as grouping variables. 
...
this the default behaviour, you have to set [group_by()](https://dplyr.tidyverse.org/reference/group_by.html)'s 
...
Let's illustrate this with our favourite flowers, and a function `species_count()` that 
counts the number of each specie after a `filter()`, and structures it as a tibble with one 
column per specie. 
