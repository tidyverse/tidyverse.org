...
However both commented and blank lines are counted in the `skip` parameter. So
to skip the header line in this example I would need to use `skip = 3`.


```r
read_csv(file, skip = 3, col_names = FALSE, comment = "#")
#> # A tibble: 2 x 2
#>   X1    X2   
#>   <chr> <chr>
#> 1 Mary  Sue  
...
By coloring the specifications we hope it will be more apparent when a column
differs from the rest, which should make spotting when the guessed specification
...
This made reading connections from small to medium datasets fast, but also
meant that the dataset had to fit into memory at least twice (once for the raw
data, once for the parsed data). It also meant that reading could not begin
