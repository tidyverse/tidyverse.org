...
According to [METACRAN](https://cranlogs.r-pkg.org) and [rdocumentation.org](https://www.rdocumentation.org/trends), readxl has over 2.7 million cumulative downloads and is consistently one of the top CRAN packages in terms of direct downloads. readxl has relatively few reverse dependencies (only about 70), so I believe the vast majority of usage occurs in users' R scripts. And these users throw an incredibly diverse set of Excel files at readxl, mostly with quiet success.

A recent failure, however, inspired an [upgrade of readxl's parsing of xlsx files](https://github.com/tidyverse/readxl/commit/eeeebf8171540a7cd14b373d20b08efbac7e3cd2). [Christine Zhang](https://twitter.com/christinezhang) did the data work behind a recent [front-page Los Angeles Times story about homeless arrests](http://www.latimes.com/local/politics/la-me-homeless-arrests-20180204-story.html). The Los Angeles Police Department provides daily arrest logs in the form of xlsx files, written by an unknown third-party tool. These files are a literal implementation the "minimal conformant SpreadsheetML package" and we had never seen such a specimen before. Both R/tidyverse/readxl and Python/pandas/xlrd refused to read these files. Undeterred, Christine took advantage of the fact that xlsx is "just" a bunch of XML files and took matters into her own hands. But I [got busy](https://github.com/tidyverse/readxl/pull/437), making sure that readxl would be ready the next time.
