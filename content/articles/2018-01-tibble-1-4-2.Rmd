...
We’re excited to announce version 1.4.2 of the *tibble* package, a minor update focusing mostly on display and performance.  This release addresses feedback we received for [version 1.4.1](tibble-1-4-1).  We added more options to configure the printing of tibbles, with better defaults. (The option names start with `pillar`, because that's the package that is actually responsible for printing; the new options are available with *pillar* 1.1.0 which will be installed automatically when you install or upgrade *tibble*.)
