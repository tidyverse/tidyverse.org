...
date: 2018-02-21
photo:
  url: https://unsplash.com/photos/MsZJPm2jvl0
  author: Terri Bleeker
categories: [package]
---
...
package: `str_glue()` and `str_glue_data()`.

*  `str_flatten()`, a wrapper for the `stri_flatten()` function, which 
flattens a character vector into a single string.

*  `str_remove()` and `str_remove_all()`, which wrap `str_replace()` and `str_replace_all()` for removing patterns from strings.

*  `str_squish()` removes whitespace from the left and right sides of strings, and converts multiple spaces or space-like characters from the _middle_ of strings into a single space.

```{r spacejam}
str_squish("\n\nString \t\nwith all \tthis     \rspace   in it.\n\n")
```

*  `str_sub()` now has an `omit_na` argument which can be used to ignore NAs and keep the original strings.
...
have been removed. So, you may see `Error : object ‘ignore.case’ is not exported by 'namespace:stringr'` during package build.
