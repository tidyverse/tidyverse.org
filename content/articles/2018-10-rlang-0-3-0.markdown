...
* `.check_assign` checks for `<-` calls passed in dots and asks the user to be explicit about the assignment. This helps detecting code typos where `=` should have been used instead of `<-`.

  
  ```r
  list3 <- function(...) dots_list(..., .check_assign = TRUE)
  ...
  <div class = "output"><pre class="knitr r">#&gt; Warning: Using `&lt;-` as argument is often a mistake.
