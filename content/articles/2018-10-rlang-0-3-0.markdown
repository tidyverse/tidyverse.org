...
This environment contains all functions exported by rlang. Its header includes the `[L]` tag to indicate that the environment is locked: you can't add or remove bindings from it. The same tag appears next to each binding to indicate that the bindings are locked and can't be changed to point to a different object. Finally, note how the type of many bindings is `<lazy>`. That's because packages are lazily loaded for performance reasons. Technically, the binding points to a *promise* that will eventually evaluate to the actual object, the first time it is accsessed.

The second print method concerns lists of environments. For instance, you can examine the search path environments with:
...
The print method for lists of environments is also used for the lists returned by `env_parents()`. This function returns all parents of a given environment. For search environments, the last parent of the list is the empty environment:
...
However it's possible to construct environments insulated from the search path. We'll use `env()` to create such an environment. Counting from rlang 0.3.0, pass an unnamed environment to specify a parent:
...
<div class = "output"><pre class="knitr r">#&gt; [[1]]   &lt;env: 0x7facbe8c6a38&gt;
#&gt; [[2]] $ &lt;env: empty&gt;
</pre></div>
