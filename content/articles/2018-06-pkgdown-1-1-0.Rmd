...
`new_process` is set to `TRUE` (as it is by default), pkgdown will be run in a
separate process to improve reproducibility.
...
be 30px, and stored in a top-level `icons/` directory. They will be embedded 
in a separate column of the reference index table, rather than inside of comments.
