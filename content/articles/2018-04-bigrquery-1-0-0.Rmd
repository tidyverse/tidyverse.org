...
I'm very pleased to announce that bigrquery 1.0.0 is now on CRAN. This package makes it easy to work with data stored in [Google BigQuery](https://developers.google.com/bigquery/), a hosted database for big data. The bigrquery package provides three levels of abstraction on top of BigQuery:
...
There are three four changes in this version of bigrquery:

* Support for repeated and nested fields, which are read into list-columns.
  
* Improved support for cross-dataset queries.

* Greatly improved download speeds thanks to downloading pages of data 
  in parallel, and custom C++ parsing code.

* A new low-level API provides a much more consistent interface
...
One of the neatest things about BigQuery is that it supports nested and repeated fields, which also also called structs (or records) and arrays. bigrquery now supports those fields, reading them into list-columns:
...
In the previous version of bigrquery it was difficult to perform queries across datasets because `dbConnect()` forced you to supply a `dataset` and only allowed you to reference tables inside that dataset. Now the dataset is optional (the only required argument to `DBI::dbConnect()` is a project to bill) and both DBI and dplyr interfaces accept qualified table names: either `dataset.table` or `project.dataset.table`.

The following example this using my test project, which contains a `basedata` dataset containing the `mtcars` table.
...
* Data is downloaded from BigQuery in pages. Previously bigrquery download then
  parsed each page. Now, bigrquery downloads all pages, then parse all pages.
  This means that you'll now see two progress bars: one for downloading JSON 
  from BigQuery and one for parsing that JSON into a data frame.
  
* Because all pages are downloaded first, we can now download in parallel, 
  using up to 6 simultaneous connections by default. This generally doesn't
  result in a six-fold speed up, but should at least double download speed.
...
I can now download the first million rows of `publicdata.samples.natality` in about a minute, about 10x faster than the previous version. This data frame takes up 170 MB of space in BigQuery and 140 MB of memory in R, so a minute to download doesn't seem unreasonable. The bottleneck for loading BigQuery data is now parsing BigQuery's json format, which is difficult to optimise further as I'm already using the fastest C++ json parser, [RapidJson](http://rapidjson.org). If this is still too slow (because you download a lot of data), see `?bq_table_download` for an alternative approach.
