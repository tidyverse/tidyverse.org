...
If you are using GitHub, the easiest way to make this your package website is to check it in, push it, then go repo settings ensure that the __GitHub pages__ source is set to "master branch /docs folder".
...
* [`init_site()`](init_site.html)
* [`build_home()`](build_home.html)
* [`build_reference()`](build_reference.html)
* [`build_articles()`](build_articles.html)
* [`build_tutorials()`](build_tutorials.html)
* [`build_news()`](build_news.html)

## In the wild

A great way to see what you can do with pkgdown is to look at existing website. As well as all the tidyverse websites, here are a few examples created by some of the contributors to pkgdown:
...
pkgdown has been a long time coming, and it wouldn't have been possible with out a devoted community of users, many of whom have gone on to contribute fixes. I'd like to particularly thank [Jay Hesselberth](https://hesselberthlab.org) who put all of working into implementing site search in partnership with [Algolia's docsearch](https://hesselberthlab.org). 
