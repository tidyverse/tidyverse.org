...
The default behaviour (`.name_repair = "check_unique`) implements `validate = TRUE`. So you can simply remove `validate = TRUE`:
...
Here are alternatives to `validate = FALSE`, if you know that `packageVersion("tibble") >= "2.0.0"`:
