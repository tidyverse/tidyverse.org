...
* [remotes](https://remotes.r-lib.org): Installing packages (i.e.
  `install_github()`).
...
* [sessioninfo](https://github.com/r-lib/sessioninfo): R session info (i.e.
  `session_info()`).
* [usethis](https://usethis.r-lib.org): Automating package setup (i.e.
  `use_test()`).
...
colorful) output to the check results, which makes finding where a check fails
much easier.
...
| `test_coverage_file()`   | <kbd>Ctrl</kbd>+<kbd>G</kbd>                    | <kbd>Command ⌘ </kbd>+<kbd>G</kbd>                    |
| `test_coverage()`        | <kbd>Ctrl</kbd>+<kbd>Shift ⇧</kbd>+<kbd>T</kbd> | <kbd>Command ⌘ </kbd>+<kbd>Shift ⇧</kbd>+<kbd>G</kbd> |
...
We are of course grateful to _all_ of the *391* people who contributed not just code, but also issues and comments for this release:
