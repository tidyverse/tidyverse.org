...
We have juse released the [cleancall] package to CRAN. [cleancall] is a
...
sure that resources acquired by a C fucntion are correctly released
...
Errors may happen two ways. They can be thrown directly by the
function. E.g. a function that reads a file will throw an error it if
fails to open the file for reading. These errors are easy to handle, we
can simply release all resource before throwing the error.
...
At the end of the function, we need to unprocect the external pointer,
and the tag:
